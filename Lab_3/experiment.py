...
class Experiment():
    ...
    def get_partial_matrix(self):
        ...
        for i in range(PARTIAL_MATR_SIZE):
            x = []
            for j in range(1, FACTORS_NUMBER - REPLICA + 1):
                index = j#(j - 1)*2 + 1
                if i // (2 ** (j - 1)) % 2 == 1:
                    matrix[i][index] = 1
                else:
            ...
            matrix[i][5] = matrix[i][1]*matrix[i][2]*matrix[i][3]
            matrix[i][6] = matrix[i][1]*matrix[i][2]*matrix[i][4]
            matrix[i][7] = matrix[i][1]*matrix[i][3]*matrix[i][4]
            matrix[i][8] = matrix[i][2]*matrix[i][3]*matrix[i][4]
